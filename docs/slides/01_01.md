...
Tailwinds isn't for beginners, it requires some tools in order to work with your projects.
...
There's another way you can use Tailwinds, and that's through the CDN, or content delivery network link. We'll be using that in this course, when I want to give you quick examples to try.
...
> >
Here's a couple of Visual Studio Code extensions you might consider installing. 
...
Then, we'll need to install some modules. Tailwinds is a postcss plugin, so we'll need postcss and we'll also install the autoprefixer to add support for older browsers. Of course we'll need tailwinds as well as some tools to run things. We're using gulp and we'll need the gulp-postcss module. Finally, we'll add browser-sync for live reloading.
...
```[1-5|7-18|20-23|25-33|35-36] const gulp = require('gulp'),
...
Tailwinds uses configuration files to set up and modify how it works. You'll need to create at least a basic one and you can do that with this command. The -p option also creates a `postcss.config.js` file, which is the same thing for postcss.
...
<img src="images/Logo.svg" alt="Binaryville Logo">
...
To load up the proper parts of Tailwinds, we'll also need a postcss starter file. Out `index.html` is looking for a file called `styles.css`, so we'll create that inside a `src/css` folder
...
<div class="btn-group mt-3" role="group" aria-label="Basic example">
  <a type="button" class="animate__animated animate__backInLeft btn btn-lg btn-exciting text-white" href="https://github.com/LinkedInLearning/tailwind-css-2841311/tree/01_01b" target="_blank">Start</a>
  <a type="button" class="animate__animated animate__zoomInDown btn btn-lg btn-royal text-white" href="htthttps://github.com/LinkedInLearning/vue3-esst-2834032/compare/01_01b..01_01e" target="_blank">Diff</a>
  <a type="button" class="animate__animated animate__backInRight animate__slow btn btn-lg btn-primary text-white" href="https://github.com/LinkedInLearning/tailwind-css-2841311/tree/01_01e" target="_blank">End</a>
</div>
